    ...
    public void addRenderable(Renderable renderable, char renderableType, int x, int y) {
        if (renderable != null) {
            if (renderableType == RenderableType.PACMAN) {
                ...
                GhostImpl ghost = (GhostImpl) renderable;
                switch (renderableType) {
                    case RenderableType.BLINKY:
                        ghost.setChaseStrategy(new BlinkyChaseStrategy());
                        blinky = ghost;
                        break;
                    case RenderableType.INKY:
                        ghost.setChaseStrategy(new InkyChaseStrategy(blinky));
                        break;
                    case RenderableType.PINKY:
                        ghost.setChaseStrategy(new PinkyChaseStrategy());
                        break;
                    case RenderableType.CLYDE:
                        ghost.setChaseStrategy(new ClydeChaseStrategy());
                        break;
                }
